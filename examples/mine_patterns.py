...
process_kwargs = {'sdef': None, 'dist_def': 'CATM', 'rsa_thresh': 0.05, 'rd_thresh': 3.03, 'distance_cutoff': 20.0, 'percent_edges': 1.0, 'edge_prune': 'DEGREE', 'num_st_dev_edges': 1.0, 'coef_alpha': 1.0, 'exp_beta': 2.3, 'r_offset': 0.0} 
...
sg = pg.subgraph_patterns['1_H#(C)C_42'] 
# Final step: Find protein subgraphs 
sg.find_protein_subgraphs('sequence')
# To switch clustering, call 'sg.set_clustering('structural')
# Visualize pattern
sg.subgraph_to_Image().show()
# Visualize protein subgraph
sg.subgraph_to_Image(id='1CC5(1)-1').show()
# Print reports
